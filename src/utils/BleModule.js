...
export default class BleModule {
  ...
  //Obtain Notify, Read, Write, WriteWithoutResponse of serviceUUID with characteristicUUID
  getUUID(services) {
    this.readServiceUUID = [];
    this.readCharacteristicUUID = [];
    this.writeWithResponseServiceUUID = [];
  ...
  write(value, index) {
    let formatValue;
    if (value === '0D0A') {
      // Send the end sign of the small ticket printer directly
      formatValue = value;
    } else {
      // Send content, convert to base64 encoding
